"""
Clean Email Service for MovedIn 2.0
Sends exactly 3 emails: Customer, Vendor, Admin
No duplicates, proper routing
"""
...
class CleanEmailService:
    """Clean email service that sends exactly 3 distinct emails"""
    ...
    def __init__(self):
        ...
        logger.info(f"📧 Clean Email Service initialized")
    ...
    def send_complete_booking_emails(self, lead_data: Dict[str, Any], lead_id: int, payment_intent_id: str = None) -> Dict[str, bool]:
        """
        Send exactly 3 emails for a complete booking:
        1. Customer confirmation email
        2. Vendor order email (to support@movedin.com for now)
        3. Admin health status email
        """
        try:
            logger.info(f"🎯 Sending complete booking emails for lead #{lead_id}")
            ...
            vendor_subject = f"📋 New Move Order #{lead_id} - {vendor_name} - MovedIn Platform"
            ...
            admin_subject = f"📊 Lead Health Status #{lead_id} - MovedIn Admin"
            ...
            logger.info(f"✅ Complete booking emails sent - Results: {results}")
            return results
            
        except Exception as e:
            logger.error(f"❌ Failed to send complete booking emails: {e}")
            return {
                "customer_email": False,
                "vendor_email": False,
                "admin_email": False
...
clean_email_service = CleanEmailService()
