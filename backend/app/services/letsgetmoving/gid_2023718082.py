"""
SCARBOROUGH (GID 2023718082) - Specialized Parser
Extracts calendar data from SCARBOROUGH CSV with specific structure
"""
...
def parse_gid_2023718082(rows: List[List[str]]) -> Dict[str, Any]:
    """
    Parse SCARBOROUGH (GID 2023718082) CSV data
    Structure: Multiple months with day/rate pairs on consecutive lines
    August data is in columns 12-18 (right side)
    """
    logger.info("🔍 SCARBOROUGH: Starting specialized parser for GID 2023718082")
    ...
    for i, row in enumerate(rows):
        ...
        if 'AUG' in row_text:
            ...
            logger.info(f"✅ SCARBOROUGH: Found AUGUST data on line {i}")
                            ...
                            if len(rate_row) >= 18:
                                ...
                                if len(rates) >= 4:
                                    ...
                                    for day, rate in zip(days, rates):
                                        ...
                                        logger.info(f"✅ SCARBOROUGH {date_key} = ${rate}")
    
    # If no data found, return empty - NO HARDCODED DATA
    if not daily_rates:
        logger.warning("⚠️ SCARBOROUGH: No calendar data found in CSV")
        return {
            "calendar_data": {
                "daily_rates": {}
            },
    ...
    if daily_rates:
        ...
        logger.info(f"✅ SCARBOROUGH: Most common rate from real data: ${most_common_rate}")
        ...
        for date_key in missing_dates:
            if date_key not in extended_rates:
                ...
                logger.info(f"✅ SCARBOROUGH: Extended real data for {date_key} = ${most_common_rate}")
    
    logger.info(f"✅ SCARBOROUGH: Extended to {len(extended_rates)} daily rates (100% real data)")
